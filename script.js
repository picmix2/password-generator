...
// Функция для безопасного отображения текста (экранирование HTML)
...
generateMultipleBtn.addEventListener('click', () => {
    ...
    if (firstPassword.includes('Выберите')) {
        ...
        multiplePasswordsEl.innerHTML = `<div class="password-item" style="color:#ff6b6b">${escapeHtml(firstPassword)}</div>`;
        return;
    }
    ...
    for (let i = 0; i < 5; i++) {
        ...
        const escapedPassword = escapeHtml(password); // Экранируем пароль
        const passwordDiv = document.createElement('div');
        passwordDiv.className = 'password-item';
        passwordDiv.innerHTML = `
            <span>${escapedPassword}</span>
            <button class="copy-small" data-password="${escapedPassword.replace(/"/g, '&quot;')}">Копировать</button>
        `;
    ...
    document.querySelectorAll('.copy-small').forEach(button => {
        button.addEventListener('click', (e) => {
            // При копировании берем оригинальный пароль (неэкранированный)
